...
# is turned on.
set GNUSTEP_MAKE_STRICT_V2_MODE=yes
...
if ( ! ${?GNUSTEP_CONFIG_FILE} ) then
  setenv GNUSTEP_CONFIG_FILE "/System/Library/Preferences/GNUstep.conf"
else
...
if ( ! ${?GNUSTEP_USER_CONFIG_FILE} ) then
  setenv GNUSTEP_USER_CONFIG_FILE "Library/Preferences/GNUstep.conf"
else
...
if ( "${GNUSTEP_MAKE_STRICT_V2_MODE}" == "yes" ) then
  ...
  if ( ! ${?GNUSTEP_SYSTEM_ROOT} ) then
    setenv GNUSTEP_SYSTEM_ROOT "/System"
  endif

  if ( ! ${?GNUSTEP_LOCAL_ROOT} ) then
    setenv GNUSTEP_LOCAL_ROOT "/Local"
  endif

  if ( ! ${?GNUSTEP_NETWORK_ROOT} ) then
    setenv GNUSTEP_NETWORK_ROOT "/Network"
  endif
...
if ( ! ${?LIBRARY_COMBO} ) then
  setenv LIBRARY_COMBO "ng-gnu-gnu"
endif

setenv GNUSTEP_HAS_PKGCONFIG "yes"

if ( ! ${?GNUSTEP_MAKEFILES} ) then
  setenv GNUSTEP_MAKEFILES "/System/Library/Makefiles"
endif
...
if ( "" == "" ) then
  setenv GNUSTEP_HOST "amd64-portbld-freebsd15.0"
  setenv GNUSTEP_HOST_CPU "amd64"
  setenv GNUSTEP_HOST_VENDOR "portbld"
  setenv GNUSTEP_HOST_OS "freebsd"
endif
